...
- Une couche « Courbes de niveau », de l'IGN, dans un nouveau thème « Territoire et environnement ». En ligne de commande : `--couches courbes`.
...
- Les couches « Cavités souterraines » et « Canalisations de matières dangereuses » sont demandées à l'adresse officielle des flux de Géorisques, `www.georisques.gouv.fr/services`, et non plus au service « risques » de `mapsref.brgm.fr`, dont la configuration est cassée depuis le 24 septembre 2026 : il renvoie une page d'erreur à toute requête, y compris à la carte du site Géorisques. Les deux zonages de PPR, que seul ce service publie, y restent, et échouent tant que le BRGM ne l'a pas rétabli ; la panne lui a été signalée.
- Sur Safari, l'option « niveaux de gris » de la page web ne faisait rien : Safari ignore le filtre de canevas qui mettait le fond en gris, sans le dire, et la carte sortait en couleur. La page convertit maintenant elle-même les tuiles, dans tous les navigateurs, avec les mêmes coefficients que l'application et la ligne de commande : le gris est le même partout.
