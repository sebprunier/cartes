...
| Documents d'urbanisme (PLU, PLU intercommunal, carte communale) | couches `plu` et `plu-prescriptions`, dessinées par l'outil | WFS, couches `wfs_du:zone_urba`, `wfs_du:secteur_cc` et `wfs_du:prescription_surf`, `_lin` et `_pct`, publiées par le Géoportail de l'urbanisme | [Géoportail de l'urbanisme](https://www.geoportail-urbanisme.gouv.fr/) | Licence Ouverte ([conditions générales d'utilisation](https://www.geoportail-urbanisme.gouv.fr/cgu/)) |
| OCS GE, artificialisation | couche `artificialisation`, dans le dernier millésime disponible pour la commune | WMTS, couches `OCSGE.ARTIF.<millésime>` | [IGNF_OCS-GE](https://cartes.gouv.fr/rechercher-une-donnee/dataset/IGNF_OCS-GE) | Licence Ouverte / Open License (compatible ODC-BY, CC-BY 2.0), avec un lien vers la licence ouverte 2.0 d'Etalab |
| Cours d'eau BCAE (bandes tampons) | couche `bcae`, superposée au fond de carte | WMTS, couche `HYDROGRAPHY.BCAE.LATEST` | [IGNF_BCAE](https://cartes.gouv.fr/rechercher-une-donnee/dataset/IGNF_BCAE) | Licence Ouverte 2.0 (déclarée par le [jeu de données](https://www.data.gouv.fr/datasets/bcae) de l'IGN) |
| Courbes de niveau | couche `courbes`, superposée au fond de carte | WMTS, couche `ELEVATION.CONTOUR.LINE` | [IGNF_COURBES-DE-NIVEAU](https://cartes.gouv.fr/rechercher-une-donnee/dataset/IGNF_COURBES-DE-NIVEAU) | Licence Ouverte / Open License (compatible ODC-BY, CC-BY 2.0) |
